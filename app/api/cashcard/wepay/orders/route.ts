...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { data: orders, error } = await sb
      .from('orders')
      .select('transaction_id, product_id, item_id, created_at, updated_at, finished_at, state, result_code, price, input_json, output_json')
      .eq('user_id', user.id)
    ...
    console.error('cashcard wepay orders fetch error', error);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    try {
      ...
      console.log('[cashcard wepay] Creating wePAY order:', {
    ...
    // บันทึกออเดอร์ลง database (cashcard ไม่ต้องใช้ input แต่ต้องส่ง input_json เป็น object)
    const { data: orderInsert, error: insertError } = await sb
      ...
      .insert({
        ...
        input_json: {}, // cashcard ไม่มี user input แต่ต้องส่งเป็น object
        finished_at: null,
    ...
    console.error('create cashcard wepay order error:', error);
