...
export default function AdminCouponsPage() {
  ...
  const handleIntegerChange = (value: string, field: keyof typeof formData) => {
    // ถ้าเป็นค่าว่าง ให้ผ่าน
    if (value === '') {
      setFormData((prev) => ({ ...prev, [field]: '' }));
      return;
    }
    // เอาเฉพาะตัวเลขและลบจุดทศนิยมออก
    ...
    <main className="mx-auto max-w-6xl px-4 py-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <AdminBackButton />
          <h1 className="text-xl font-semibold">จัดการคูปองส่วนลด</h1>
        </div>
                ...
                <Input
                  ...
                  onWheel={(e) => {
                    if (document.activeElement === e.currentTarget) {
                      ...
                      const step = 1;
                      const currentValue = parseInt(formData.discount_value, 10) || 0;
                      const delta = e.deltaY > 0 ? -step : step;
                      const min = parseInt(input.min || '1', 10);
                      const max = formData.discount_type === 'percent' ? 100 : parseInt(input.max || '999999', 10);
                      const newValue = Math.max(min, Math.min(max, currentValue + delta));
                      setFormData({ ...formData, discount_value: newValue.toString() });
                    }
                  }}
                  required
                />
                  ...
                  <Input
                    ...
                    onWheel={(e) => {
                      if (document.activeElement === e.currentTarget) {
                        ...
                        const input = e.currentTarget;
                        const step = 1;
                        const currentValue = parseInt(formData.min_discount, 10) || 0;
                        const delta = e.deltaY > 0 ? -step : step;
                        const min = parseInt(input.min || '0', 10);
                        const newValue = Math.max(min, currentValue + delta);
                        setFormData({ ...formData, min_discount: newValue.toString() });
                      }
                    }}
                    placeholder="0"
                  />
                ...
                <Input
                  ...
                  onWheel={(e) => {
                    if (document.activeElement === e.currentTarget) {
                      ...
                      const input = e.currentTarget;
                      const step = 1;
                      const currentValue = parseInt(formData.min_purchase, 10) || 0;
                      const delta = e.deltaY > 0 ? -step : step;
                      const min = parseInt(input.min || '0', 10);
                      const newValue = Math.max(min, currentValue + delta);
                      setFormData({ ...formData, min_purchase: newValue.toString() });
                    }
                  }}
                  placeholder="0"
                />
