...
export default function MtopupDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Debug: ตรวจสอบ inputs
        console.log('[MtopupDetail] inputs from API:', merged.inputs);
        setData(merged);
  ...
  const onSubmit = async () => {
    ...
    setSubmitMsg(null);
    try {
      // Map input keys จาก database ไปยัง phone_number สำหรับ API
      const phoneInput = data.inputs?.find(inp => inp.key === 'phone_number');
      const phoneValue = phoneInput ? input[phoneInput.key] : null;
      ...
      if (!res.ok) {
        ...
        // แปลง error message ให้เป็น string
        ...
        if (json?.detail) {
          if (typeof json.detail === 'string') {
            ...
            errorMsg = json.detail.message || json.detail.error || JSON.stringify(json.detail);
          }
  ...
  if (error) return <main className="mx-auto max-w-6xl px-4 py-6 text-blue-500">{error}</main>;
      ...
      <div className="relative overflow-hidden py-12 md:py-16 shadow-lg">
        ...
        <div className="relative mx-auto max-w-6xl px-4 mb-4">
          <div className="flex items-center gap-2 text-sm text-white/80">
            ...
            <span>/</span>
            <Link href="/mtopup" className="hover:text-white transition-colors flex items-center gap-1">
        ...
        <div className="relative mx-auto max-w-6xl px-4">
          <div className="flex flex-col md:flex-row items-center gap-6 text-white">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-white/90 text-sm md:text-base">Mobile Topup {data.name}</p>
              ...
              <section className="bg-[#0a0a0a] rounded-xl shadow-sm border border-gray-800 p-6">
                ...
                <div className="space-y-4">
                  {data.inputs.map((inp, idx) => (
                    <div key={inp.key || idx}>
                      <label className="block text-sm font-medium text-gray-300 mb-2">{inp.title}<span className="text-blue-500">*</span></label>
                      {inp.type === 'select' ? (
                        <select 
                          className="w-full px-4 py-3 rounded-lg border border-gray-700 bg-[#1a1a1a] text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                          value={input[inp.key] || ''} 
                        ...
                        <input 
                          className="w-full px-4 py-3 rounded-lg border border-gray-700 bg-[#1a1a1a] text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                ...
                {data.items.map((it) => (
                  <button 
                    ...
                    className={`relative rounded-2xl border-2 transition-all duration-200 overflow-hidden group ${
                      sku === it.sku 
                        ? 'border-blue-600 shadow-lg ring-4 ring-blue-900/50 scale-[1.02]' 
                        : 'border-gray-700 bg-[#0a0a0a] hover:border-blue-500 hover:shadow-md'
                    }`}
                  >
                    {/* Selected Indicator */}
                    {sku === it.sku && (
                    ...
                    <div className="p-4 text-center bg-[#0a0a0a] relative">
                      ...
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-xl font-bold text-blue-500 tabular-nums">{Number(it.price).toFixed(2)}฿</span>
                        {(() => {
          ...
          <div className="space-y-4">
            ...
            <section className="bg-[#0a0a0a] rounded-xl shadow-sm border border-gray-800 p-6">
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-2xl font-bold text-blue-500 tabular-nums">{animatedFinalPrice.toFixed(2)}฿</span>
              </div>
              
              <Button 
                className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-bold py-3 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200" 
