...
export const metadata: Metadata = {
  title: 'นโยบายความเป็นส่วนตัว - WeXPlus',
  description: 'นโยบายความเป็นส่วนตัวของ WeXPlus - ข้อมูลเกี่ยวกับการเก็บรวบรวม ใช้ และปกป้องข้อมูลส่วนบุคคล',
};
...
async function getPrivacyData() {
  ...
  try {
    ...
    console.error('[Privacy Policy] Exception:', err);
    return {
      settings: { title: 'นโยบายความเป็นส่วนตัว', description: null },
      items: [],
      updateDate: '',
    };
  }
...
export default async function PrivacyPage() {
  const { settings, items, updateDate } = await getPrivacyData();
  const sections = items.map((item, index) => {
    // ลบเลขหน้าออกจาก title (เช่น "1. การเก็บรวบรวมข้อมูล" -> "การเก็บรวบรวมข้อมูล")
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 scroll-smooth">
      {/* Main Banner */}
      <div className="bg-gradient-to-r from-emerald-600 via-emerald-500 to-emerald-600 border-b-4 border-emerald-400/50">
        <div className="mx-auto max-w-7xl px-4 py-12 md:py-16">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <p className="text-white/80 text-sm md:text-base">Privacy Policy - WeXPlus</p>
            </div>
          </div>
        </div>
      </div>
                ...
                <nav className="space-y-2">
                  {sections.map((section, index) => (
                    <a
                      key={section.id}
                      href={`#${section.id}`}
                      className="block px-3 py-2 text-sm text-gray-300 hover:text-emerald-400 hover:bg-emerald-500/20 rounded-md transition-colors scroll-smooth"
                    >
          ...
          <div className={sections.length > 0 ? 'lg:col-span-3' : 'lg:col-span-4'}>
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 md:p-8 lg:p-10">
              {settings.description && (
                <div className="mb-8 p-4 rounded-lg bg-emerald-900/20 border border-emerald-800/50">
              ...
              {items.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400 text-lg mb-2">ยังไม่มีเนื้อหานโยบายความเป็นส่วนตัว</p>
