...
export default function PrivacyContent() {
  ...
  const loadData = async () => {
    try {
      ...
      const [settingsRes, itemsRes] = await Promise.all([
        fetch('/api/admin/privacy/settings'),
        fetch('/api/admin/privacy/items'),
      ]);
  ...
  const handleOpenDialog = (item?: PrivacyItem) => {
    if (item) {
      setEditingItem(item);
  ...
  const handleSaveItem = async () => {
    ...
    try {
      ...
      const url = editingItem
        ? `/api/admin/privacy/items/${editingItem.id}`
        : '/api/admin/privacy/items';
      ...
      toast.show({
        title: 'บันทึกสำเร็จ',
        description: editingItem ? 'แก้ไขนโยบายเรียบร้อยแล้ว' : 'เพิ่มนโยบายเรียบร้อยแล้ว',
      });
  ...
  const handleDeleteItem = async (id: string) => {
    if (!confirm('คุณแน่ใจหรือไม่ว่าต้องการลบนโยบายนี้?')) {
      return;
    }

    try {
      setSaving(true);
      const res = await fetch(`/api/admin/privacy/items/${id}`, {
        method: 'DELETE',
      });
      ...
      toast.show({
        title: 'ลบสำเร็จ',
        description: 'ลบนโยบายเรียบร้อยแล้ว',
      });
    ...
    try {
      ...
      await Promise.all(
        updatedItems.map((item) =>
          fetch(`/api/admin/privacy/items/${item.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
    ...
    <div className="space-y-6">
      ...
      <div className="card p-6 space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-400 mb-4">ตั้งค่าหัวข้อและรายละเอียดสำหรับหน้านโยบายความเป็นส่วนตัว</p>
        </div>

        <div className="space-y-4">
          <div className="grid gap-2">
            <Label htmlFor="privacy-title">หัวข้อหลัก</Label>
            <Input
              id="privacy-title"
          ...
          <div className="grid gap-2">
            <Label htmlFor="privacy-description">รายละเอียดหัวข้อหลัก (ไม่บังคับ)</Label>
            <Textarea
              id="privacy-description"
              value={settings.description || ''}
              onChange={(e) => setSettings({ ...settings, description: e.target.value || null })}
              placeholder="อธิบายรายละเอียดเกี่ยวกับนโยบายความเป็นส่วนตัว..."
              rows={3}
              className="bg-[#1a1a1a] border-gray-700"
            />
          </div>
      ...
      {/* รายการ Privacy Items */}
      <div className="card p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-base font-semibold mb-2">รายการนโยบาย</h3>
            <p className="text-xs text-gray-400">จัดการนโยบายแต่ละข้อ</p>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>
                  {editingItem ? 'แก้ไขนโยบาย' : 'เพิ่มนโยบายใหม่'}
                </DialogTitle>
              </DialogHeader>
              <div className="space-y-4 pt-4">
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    placeholder="เช่น 1. การเก็บรวบรวมข้อมูล"
                    className="bg-[#1a1a1a] border-gray-700"
                  />
                </div>
                <div className="grid gap-2">
                  ...
                  <Textarea
                    ...
                    placeholder="กรอกรายละเอียดของนโยบายนี้..."
                    rows={10}
                    className="bg-[#1a1a1a] border-gray-700"
                  />
        ...
        {items.length === 0 ? (
          <div className="text-center py-12 border border-gray-800 rounded-lg">
            <p className="text-gray-400 mb-4">ยังไม่มีนโยบาย</p>
            ...
            {items.map((item) => (
              <div
                ...
                className={`p-4 rounded-lg border border-gray-800 bg-gray-900/30 hover:bg-gray-900/50 transition-colors cursor-move ${
                  draggedItem === item.id ? 'opacity-50 border-blue-500' : ''
